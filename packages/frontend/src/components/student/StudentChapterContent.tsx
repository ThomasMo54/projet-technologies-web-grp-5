...
/**
 * Affiche le contenu d'un chapitre pour l'étudiant
 * 
 * Fonctionnalités :
 * - Contenu HTML (Quill)
 * - Quiz avec états : non commencé / réussi / échoué
 * - Modale quiz qui reste ouverte après soumission
 * - Rafraîchissement des données après quiz
 */
const StudentChapterContent: React.FC<StudentChapterContentProps> = ({ chapter }) => {
  ...
  /**
   * Appelé quand le quiz est terminé
   * Rafraîchit les données + garde la modale ouverte
   */
  const handleQuizComplete = async (_: number) => {
    ...
    // Modale reste ouverte pour voir les résultats
    setViewResultsMode(true);
  };

  /** Ouvre la modale en mode résultats */
  const handleViewResults = () => {
    setViewResultsMode(true);
    setQuizModalOpen(true);
  };

  /** Ouvre la modale en mode passage */
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400">
                      Score: {userAnswer.score}% (Need {passingScore}%)
                    </p>
                  </div>
                </div>
      ...
      {/* === Modale du quiz (reste ouverte après submit) === */}
      <Modal 
      ...
      >
        {quiz && (
          <StudentQuizComponent 
            quiz={quiz} 
            onComplete={handleQuizComplete}  // garde la modale ouverte
