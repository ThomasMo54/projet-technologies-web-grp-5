...
interface CourseChatbotProps {
  courseId: string; // ID du cours pour le contexte IA
...
/**
 * Chatbot IA intégré au cours
 * Fonctionnalités :
 * - Envoi de questions sur le contenu du cours
 * - Réponses en temps réel via API
 * - Historique des messages avec horodatage
 * - Scroll automatique vers le bas
 * - Indicateur de chargement
 * - Design moderne avec avatars et bulles
 * Utilise React Query pour les appels API
 */
const CourseChatbot: React.FC<CourseChatbotProps> = ({ courseId, onClose }) => {
  const [question, setQuestion] = useState(''); // Question en cours de saisie
   ...
   * Ajoute le message utilisateur + déclenche la requête
   */
  const handleSend = () => {
  ...
  return (
    <div 
      className="fixed bottom-6 right-6 z-50 flex flex-col shadow-2xl rounded-2xl overflow-hidden bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
      style={{ 
        width: '450px', 
        height: '600px',
